...
export function PoolCalculator({
  ...
  useEffect(() => {
    try {
      ...
      // O cálculo continua funcionando se o navegador bloquear o armazenamento.
    }
  }, [])
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(
        "rhyno-pool-calculator",
        JSON.stringify(state),
      )
    } catch {
      // Os dados só não serão mantidos após atualizar a página.
    }
  }, [state])
  ...
  const entriesTotal = activeGroups.reduce(
    (total, group) =>
      total +
      group.events.reduce(
  ...
  async function saveBattle(password: string) {
    ...
    try {
      ...
      setAdminError(
        error instanceof Error ? error.message : "Erro ao salvar rinha",
      )
    } finally {
      setSavingGroup(null)
    }
  ...
  return (
    <section className="rounded-3xl border border-border bg-card/60 p-4 backdrop-blur sm:p-5 lg:col-span-2">
      ...
      <p className="mt-2 max-w-3xl text-xs text-muted-foreground sm:text-sm">
        Informe em reais quanto cada aposta retornou do cassino e marque a
        equipe vencedora. O sistema soma o pote, retira 15% e divide o prêmio
        entre os bilhetes da vencedora.
      </p>
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-2">
        {activeGroups.length === 0 && (
          <p className="text-sm text-muted-foreground">
            Nenhum confronto disponível para calcular.
          </p>
        )}
        ...
        {activeGroups.map((group) => {
          ...
          const grossPool = group.events.reduce(
            (sum, event) =>
              sum +
              parseMoney(groupState?.casinoReturns[event.id] ?? ""),
            0,
          )

          const fee = grossPool * FEE_RATE
          const netPool = grossPool - fee
          const winner = group.events.find(
            (event) => event.id === groupState?.winnerId,
          )

          const winnersCount = winner
            ? participants
                .filter(
                  (participant) => participant.eventId === winner.id,
                )
                .reduce(
                  (sum, participant) =>
                    sum + Math.max(1, participant.quantity),
                  0,
                )
            : 0

          const prizePerTicket =
            winnersCount > 0 ? netPool / winnersCount : 0
          ...
          return (
            <article
              key={group.id}
              className="min-w-0 rounded-2xl border border-border bg-background/40 p-3 sm:p-4"
            >
                    ...
                    >
                      ...
                      <label className="relative block">
                        ...
                        <input
                          inputMode="decimal"
                          value={
                            groupState?.casinoReturns[event.id] ?? ""
                          }
                          onChange={(e) =>
                            updateReturn(
                              group.id,
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          selectWinner(group.id, event.id)
                        }
                        className={
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
                ...
                <Result
                  label="Pote líquido"
                  value={formatBRL(netPool)}
                  highlight
                />
                <Result
                  label={
                    winnersCount > 0
                      ? `Por vencedor (${winnersCount})`
                      : "Por vencedor"
                  }
                  value={
                    winnersCount > 0
                      ? formatBRL(prizePerTicket)
                      : "Marque a equipe"
                  }
                  highlight
                />
...
function Result({
  ...
  return (
    <div className="min-w-0 rounded-xl border border-border bg-card/60 p-2.5 sm:p-3">
      ...
      <p
        className={
          "mt-1 break-words text-xs font-extrabold sm:text-sm " +
