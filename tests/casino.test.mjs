...
test("interface real comporta todos os times sem renderizar dados sensíveis", async () => {
  ...
  try {
    ...
    assert.match(squad, /Roda da guild/)
    assert.match(squad, /Partiu hunt com o Dukoth\?/)
    assert.match(squad, /Quem vai pra cave agora\?/)
    assert.match(squad, /Meu bilhete entrou na guild!/)
    assert.equal((squad.match(/aventureiro do bilhete na equipe/g) ?? []).length, 6)
